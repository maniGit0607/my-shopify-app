...
api.post('/graphql', async (c) => {
  try {
    ...
    // Strategy: Try stored token first (from OAuth), then token exchange
    const storage = new KVStorage(c.env.SHOP_TOKENS);
    const storedTokenData = await storage.getShopToken(shop);
    
    let accessToken: string | null = null;

    if (storedTokenData) {
      // Use stored token from OAuth flow
      console.log('[GraphQL] Using stored OAuth token');
